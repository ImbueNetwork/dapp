...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema
    .createTable(usersTableName, (builder) => {
      /**
  ...
  await knex.schema
    .createTable(web3AccountsTableName, (builder) => {
  ...
  await knex.schema
    .createTable(federatedCredsTableName, (builder) => {
      builder.integer("id");
      builder.text("issuer");
  ...
  await knex.schema
    .createTable(projectStatusTableName, (builder) => {
      builder.increments("id", { primaryKey: true });
      builder.text("status");
      auditFields(knex, builder);
    })
  ...
  await knex.schema
    .createTable(projectsTableName, (builder: Knex.CreateTableBuilder) => {
      builder.increments("id", { primaryKey: true });
  ...
  await knex.schema
    .createTable(milestonesTableName, (builder) => {
