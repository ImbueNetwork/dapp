...
export async function seed(knex: Knex): Promise<void> {
  ...
  let clients = ["imbue", "mangata", "oak", "nft"];
  let languages = [
    "french",
    "german",
    "dutch",
        ...
        for (let d = 1; d < services.length; d++) {
          ...
          await knex("users")
            .insert([
              {
                display_name:
                  skills[a] + "_" + clients[b] + "_" + services[d] + rbig,
                username:
                  skills[a].replace(" ", "_") +
                  "_" +
                  clients[b].replace(" ", "_") +
                  "_" +
                  services[d].replace(" ", "_") +
                  rbig,
                email: skills[a] + languages[c] + rbig + "@gmail.com",
                password:
                  a < 2 ? bcrypt.hashSync("testpassword") : "testpassword",
              },
            ])
            .then(async () => {
              await knex("freelancers")
                .insert({
                  freelanced_before:
                  ...
                  bio:
                    ...
                    services[d] +
                    " and have many clients including " +
                    clients[b],
