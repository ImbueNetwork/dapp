...
// @ts-ignore
export const polkadotJsStrategy = new JwtStrategy(jwtOptions, async function (
  jwt_payload,
  next
) {
  ...
  try {
    ...
    if (!user) {
      ...
      user.web3Accounts = await db<Web3Account>("web3_accounts")
        .select()
        .where({
          user_id: user.id,
        });
...
polkadotJsAuthRouter.post("/", (req, res, next) => {
  ...
  try {
    ...
    const err = new Error("Invalid `address` param.", {
      cause: e as Error,
    });
...
polkadotJsAuthRouter.post("/callback", (req, res, next) => {
  db.transaction(async (tx) => {
    try {
      const solution: Solution = req.body;
      const web3Account = await fetchWeb3Account(solution.address)(tx);
      ...
      await tx.rollback();
      next(new Error(`Unable to finalise login`, { cause: e as Error }));
    }
  });
});
