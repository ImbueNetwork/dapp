...
export const fetchWeb3Account = (address: string) => (tx: Knex.Transaction) =>
    tx<Web3Account>("web3_accounts").select().where({ address }).first();
...
export const generateGetStreamToken = async (user: User) => {
    if (
        ...
        process.env.REACT_APP_GETSTREAM_SECRET_KEY
    ) {
...
export const fetchBriefApplications = (id: string) => (tx: Knex.Transaction) =>
...
export const fetchBrief = (id: string) => (tx: Knex.Transaction) =>
...
export const fetchAllBriefs = () => (tx: Knex.Transaction) =>
    tx
        .select(
            ...
            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(industries.name as text)) as industries"
            ),
            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(industries.id as text)) as industry_ids"
            )
        )
        .from("briefs")
        .leftJoin("brief_industries", {
            "briefs.id": "brief_industries.brief_id",
        })
        .leftJoin("industries", {
            "brief_industries.industry_id": "industries.id",
        })
...
export const upsertItems =
    (items: string[], tableName: string) => async (tx: Knex.Transaction) => {
        var item_ids: number[] = [];
        try {
            //TODO Convert to map
            for (const item of items) {
                ...
                if (!existing_item) {
                    item_id = await (
                        await insertToTable(item, tableName)(tx)
                    ).id;
...
export const getOrCreateFederatedUser = (
    ...
    db.transaction(async (tx) => {
        ...
        try {
             ...
             */
            const federated = await tx<FederatedCredential>(
...
export const fetchFreelancerDetailsByUserID =
    (user_id: number | string) => (tx: Knex.Transaction) =>
        fetchAllFreelancers()(tx).where({ user_id }).first().debug(false);

export const fetchFreelancerDetailsByUsername =
    (username: string) => (tx: Knex.Transaction) =>
...
export const fetchAllFreelancers = () => (tx: Knex.Transaction) =>
    tx
        .select(
            ...
            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(languages.name as text)) as languages"
            ),
            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(languages.id as text)) as language_ids"
            ),

            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(services.name as text)) as services"
            ),
            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(services.id as text)) as service_ids"
            ),

            tx.raw("ARRAY_AGG(DISTINCT CAST(clients.name as text)) as clients"),
            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(clients.id as text)) as client_ids"
            ),

            tx.raw(
                "ARRAY_AGG(DISTINCT CAST(clients.img as text)) as client_images"
            ),
            tx.raw(
        ...
        .leftJoin("services", {
            "freelancer_services.service_id": "services.id",
        })
    ...
    fetchAllFreelancers()(tx)
        ...
        .where(function () {
            if (filter.services_range.length > 0) {
                this.whereIn(
                    "freelancer_services.service_id",
                    filter.services_range
                );
            }
        })
        .where(function () {
