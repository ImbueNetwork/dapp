...
import {
  validateUserFromJwt,
  verifyUserIdFromJwt,
} from "../../../middleware/authentication/strategies/common";
...
router.get("/", (req, res, next) => {
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to fetch all freelancers`, {
          cause: e as Error,
        })
      );
    }
...
router.get("/:username", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to fetch freelancer details by userid: ${username}`, {
          cause: e as Error,
        })
      );
    }
...
router.post("/", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      if (!freelancer_id) {
        return next(new Error("Failed to insert freelancer details."));
      }
...
router.put("/:username", async (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      const exists: any = await models.fetchFreelancerDetailsByUsername(
        username
      )(tx);
      if (!exists) {
        return next(new Error("Freelancer does not exist."));
      }
      await models.updateFreelancerDetails(exists.user_id, freelancer)(tx);
      let updated_freelancer_details =
        await models.fetchFreelancerDetailsByUsername(username)(tx);
      return res.send(updated_freelancer_details);
    } catch (e: any) {
      return next(
        new Error(`Failed to update freelancer details: ${e.message}`)
      );
    }
  });
});
...
router.post("/search", (req, res, next) => {
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to search all freelancers`, {
          cause: e as Error,
        })
      );
    }
