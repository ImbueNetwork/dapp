    ...
    if (details != null) {
      db.transaction(async (tx) => {
        try {
          ...
          const updated = await models.updateMilestoneDetails(
            id,
            milestoneId,
            details
          )(tx);
          ...
          next(
            new Error(`Failed to update milestones`, {
...
router.get("/:id/milestones", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to fetch all milestones`, {
          cause: e as Error,
        })
      );
    }
...
router.get("/:id/milestone/:milestoneId", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to fetch the milestone`, {
          cause: e as Error,
        })
      );
    }
  ...
  (req, res, next) => {
    ...
    db.transaction(async (tx) => {
      try {
        ...
        next(
          new Error(`Failed to insert milestone.`, {
            cause: cause as Error,
          })
        );
      }
