...
router.get("/", (req, res, next) => {
  db.transaction(async (tx) => {
    try {
      ...
      next(new Error(`Failed to fetch all projects`, { cause: e as Error }));
    }
  });
});
  ...
  (req, res, next) => {
    ...
    db.transaction(async (tx) => {
      try {
        ...
        if (!project.id) {
          return next(
            new Error("Failed to insert milestones: `project_id` missing.")
          );
        }
        ...
        next(
          new Error(`Failed to insert project.`, {
  ...
  (req, res, next) => {
    ...
    db.transaction(async (tx) => {
      try {
        ...
        if (!project.id) {
          return next(
            new Error("Cannot update milestones: `project_id` missing.")
          );
        }
...
router.put(
  ...
  (req, res, next) => {
    ...
    const { faq } = req.body.properties as models.ProjectProperties;
    ...
    db.transaction(async (tx) => {
      try {
        ...
        next(
          new Error(`Failed to update project properties.`, {
            cause: cause as Error,
          })
        );
      }
