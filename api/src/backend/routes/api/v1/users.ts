...
router.get("/:id/project", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to fetch projects for user id: ${id}`, {
          cause: e as Error,
        })
      );
    }
    ...
    try {
      ...
      const briefsUnderReview = await Promise.all(
        pendingReviewBriefs.map(async (brief) => {
          return {
            ...brief,
            number_of_applications: (await fetchBriefApplications(brief.id)(tx))
              .length,
          };
        })
      );
      ...
      next(
        new Error(`Failed to fetch all briefs for user id ${id}`, {
          cause: e as Error,
        })
      );
    }
...
router.post("/", (req, res, next) => {
  ...
  let updateUserDetails = async (err: Error, user: User) => {
    ...
    db.transaction(async (tx) => {
      try {
        ...
        tx.rollback();
        next(
          new Error(`Unable to upsert details for user: ${username}`, {
            cause: e as Error,
          })
        );
      }
  ...
  getOrCreateFederatedUser("Imbue Network", email, username, updateUserDetails);
});
...
router.get("/:userOrEmail", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      next(
        new Error(`Failed to fetch user ${userOrEmail}`, {
          cause: e as Error,
        })
      );
    }
...
router.get("/byid/:id", (req, res, next) => {
  ...
  db.transaction(async (tx) => {
    try {
      ...
      next(new Error(`Failed to fetch user ${id}`, { cause: e as Error }));
    }
  });
});
