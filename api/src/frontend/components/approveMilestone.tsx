...
import type * as polkadot from "../utils/polkadot";
import {
...
export interface ApproveMilestoneProps {
...
interface ApproveMilestoneState {
...
export const ApproveMilestone = ({
  ...
  chainService,
}: ApproveMilestoneProps): JSX.Element => {
  ...
  const [percentageOfContributorsVoted, setPercentageOfContributorsVoted] =
    useState(-1);
  ...
  const haveAllUsersVotedOnMilestone = async (): Promise<boolean> => {
    ...
    const milestoneVotes: any = (
      await imbueApi.imbue?.api.query.imbueProposals.milestoneVotes([
        projectOnChain.id,
        firstPendingMilestoneIndex,
      ])
    ).toHuman();
    ...
    const _percentageOfContributorsVoted =
      (totalVotes / totalContributions) * 100;
    const _approveButtonEnabled = totalVotes == totalContributions;
    const _displayVotingProgress =
  ...
  const approveMilestone = async (
    ...
    const result: BasicTxResponse = await chainService.approveMilestone(
      account,
      projectOnChain,
      firstPendingMilestoneIndex
    );
  ...
  return projectOnChain && projectOnChain.milestones ? (
    <div>
      ...
      <form
        id="approve-milestone-form"
        name="approve-milestone-form"
        method="get"
        className="form"
