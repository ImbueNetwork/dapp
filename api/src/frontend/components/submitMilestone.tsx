...
export interface SubmitMilestoneProps {
...
export class SubmitMilestone extends React.Component<SubmitMilestoneProps> {
  ...
  async submitMilestone(account: InjectedAccountWithMeta): Promise<void> {
    ...
    const result: BasicTxResponse =
      await this.props.chainService.submitMilestone(
        account,
      ...
      return (
        <div>
          <ErrorDialog
            ...
            showDialog={this.state.showErrorDialog}
          ...
          >
            <Select
              ...
              onChange={(event: React.FormEvent) => {
                this.updateMilestoneValue(
                  parseInt((event.target as HTMLInputElement).value)
                );
              }}
              icon="how_to_vote"
            >
              {this.props.projectOnChain.milestones
            ...
            <button
              ...
              className={
                this.state.buttonState == ButtonState.Saving
                  ? "button primary blob"
                  : this.state.buttonState == ButtonState.Done
