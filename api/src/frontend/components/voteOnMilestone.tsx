...
export interface VoteOnMilestoneProps {
...
interface VoteOnMilestoneState {
        ...
        <h3 id="project-state-headline">
          <AccountChoice
            accountSelected={async (account) => {
              await submitVote(account);
            }}
          />
        </h3>
      ) : null}
      <form
        id="vote-submission-form"
        name="vote-submission-form"
        method="get"
        className="form"
      ...
      >
        <Select
          ...
          onChange={(event: React.FormEvent) => {
            setVote(
              Boolean(parseInt((event.target as HTMLInputElement).value))
            );
          }}
          icon="how_to_vote"
        >
