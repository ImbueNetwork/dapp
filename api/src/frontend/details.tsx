...
const initiatorHeadingMapping: Record<ProjectState, JSX.Element> = {
  ...
  [ProjectState.PendingProjectApproval]: (
    <h3>
      Funding for this project is not yet open. Check back soon for more
      updates!
    </h3>
  ),

  /* PROJECT IS PENDING FUNDING APPROVAL ROUND */
  [ProjectState.PendingFundingApproval]: (
    <h3>
      Pending funding approval. Funding round not complete or approved. Please{" "}
      <a href="https://discord.gg/jyWc6k8a">contact the team</a> for review.
    </h3>
  ),

  /* PROJECT IS IN THE CONTRIBUTION ROUND */
  [ProjectState.OpenForContribution]: (
    <h3>
      Your proposal has been created successfully and added to the funding
      round. Contributors can now fund your project
    </h3>
  ),
  ...
  [ProjectState.PendingMilestoneApproval]: (
    <h3>
      Pending milestone approval. Milestone voting round not complete or
      approved. Please{" "}
      <a href="https://discord.gg/jyWc6k8a">contact the team</a> for review.
    </h3>
  ),
  ...
  [ProjectState.PendingMilestoneSubmission]: (
    <h3>Pending milestone submission</h3>
  ),
...
const contributorHeadingMapping: Record<ProjectState, JSX.Element> = {
  ...
  /* PROJECT IS IN THE CONTRIBUTION ROUND */
  [ProjectState.OpenForContribution]: (
    <h3>This project is open for contributions!</h3>
  ),
  ...
  /* PROJECT IS IN THE VOTING ROUND */
  [ProjectState.OpenForVoting]: (
    <h3>Project contributors can now vote on milestones</h3>
  ),
...
class Details extends React.Component<DetailsProps, DetailsState> {
  ...
  async setProjectState(projectOnChain: ProjectOnChain): Promise<void> {
    let userIsInitiator = await this.props.chainService.isUserInitiator(
      this.props.user,
      projectOnChain
    );
    ...
    let lastApprovedMilestoneIndex =
      this.props.chainService.findLastApprovedMilestone(projectOnChain);
    let firstPendingMilestoneIndex =
      this.props.chainService.findFirstPendingMilestone(projectOnChain);

    const projectState = projectOnChain.projectState;

    if (userIsInitiator) {
      // SHOW WITHDRAW AND MILESTONE SUBMISSION components
      showSubmitMilestoneComponent =
        projectOnChain.fundingThresholdMet && firstPendingMilestoneIndex >= 0;
      showWithdrawComponent =
        lastApprovedMilestoneIndex >= 0 &&
        projectOnChain.raisedFunds > projectOnChain.withdrawnFunds;
      showApproveMilestoneComponent =
        projectOnChain.fundingThresholdMet && firstPendingMilestoneIndex >= 0;
    } else {
      switch (projectState) {
  ...
  async componentDidMount() {
    if (this.props.projectId) {
      const project: ProjectOnChain = await this.props.chainService.getProject(
      ...
      return (
        <div id="details">
          <div className="top-wrapper">
            <header className="project-name-header">
              <h1 className="heading-8 project-name-heading">
                <span id="project-name">{this.state.projectOnChain.name}</span>
              </h1>
            </header>
            <img
              id="project-logo"
          ...
          <FundingInfo
            ...
            lastApprovedMilestoneIndex={this.state.lastApprovedMilestoneIndex}
          ></FundingInfo>
            ...
            {this.state.showVoteComponent ? (
              <VoteOnMilestone
                ...
                firstPendingMilestoneIndex={
                  this.state.firstPendingMilestoneIndex
                }
                chainService={this.props.chainService}
              ></VoteOnMilestone>
            ) : null}
          ...
          >
            ...
            <Tab icon="money" label="Contributors" />
          </TabBar>
          ...
          <div id="tab-content-container">
            ...
            >
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: marked.parse(
                    `${this.state.projectOnChain.description}`
                  ),
                }}
              ></div>
              <ul className="project-details">
                <li className="project-detail">
                  <span className="detail-value hidden" id="in-block"></span>
                </li>
                <li className="project-detail">
                  <span className="detail-label"></span>
                  <span
                    id="project-detail-currency"
                    className="imbu-currency-label"
                  >
                    ${this.state.projectOnChain.currencyId as Currency}
                  </span>{" "}
                  <span id="funds-required">
                    {this.state.projectOnChain.requiredFundsFormatted.toLocaleString()}
                  </span>
                ...
                <li className="project-detail">
                  <span className="detail-value" id="project-website">
                    <a href={this.state.projectOnChain.website} target="_blank">
            ...
            <div
              className={`tab-content ${
                this.state.activeTabIndex === 2 ? "active" : ""
              }`}
            >
              <List twoLine id="FAQ">
                <h2>Coming soon.....</h2>
            ...
            <div
              className={`tab-content ${
                this.state.activeTabIndex === 3 ? "active" : ""
              }`}
            >
            ...
            <div
              className={`tab-content ${
                this.state.activeTabIndex === 4 ? "active" : ""
              }`}
            >
            ...
            <div
              className={`tab-content ${
                this.state.activeTabIndex === 5 ? "active" : ""
              }`}
              style={{ color: "white" }}
            >
              <DataTable>
...
document.addEventListener("DOMContentLoaded", async (event) => {
  ...
  ReactDOMClient.createRoot(document.getElementById("project-body")!).render(
    <Details
      imbueApi={imbueApi}
      projectId={projectId}
      user={user}
