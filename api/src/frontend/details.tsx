...
export const Details = ({
  ...
  const [lastApprovedMilestoneIndex, setLastApprovedMilestoneIndex] =
    useState(-1);
  const [firstPendingMilestoneIndex, setFirstPendingMilestoneIndex] =
    useState(-1);
  ...
  const setProjectState = async (
    _projectOnChain: ProjectOnChain
  ): Promise<void> => {
    const _userIsInitiator = await chainService.isUserInitiator(
      user,
      _projectOnChain
    );
    ...
    const projectMilestones = await chainService.getProjectMilestones(
      _projectOnChain
    );

    const _lastApprovedMilestoneIndex =
      await chainService.findLastApprovedMilestone(projectMilestones);
    const _firstPendingMilestoneIndex =
      await chainService.findFirstPendingMilestone(projectMilestones);
    ...
    if (_userIsInitiator) {
      ...
      _submitVisible =
        _projectOnChain.fundingThresholdMet && _firstPendingMilestoneIndex >= 0;
      _withdrawVisible =
        _lastApprovedMilestoneIndex >= 0 &&
        _projectOnChain.raisedFunds > _projectOnChain.withdrawnFunds;
      _approvedVisible =
        _projectOnChain.fundingThresholdMet && _firstPendingMilestoneIndex >= 0;
    } else {
      switch (projectState) {
  ...
  return projectOnChain != null && projectOnChain.milestones ? (
    <div id="details">
      <div className="top-wrapper">
        <header className="project-name-header">
          <h1 className="heading-8 project-name-heading">
            <span id="project-name">{projectOnChain.name}</span>
          </h1>
        </header>
      ...
      {userIsInitiator
        ? initiatorHeadingMapping[projectOnChain.projectState]
        : contributorHeadingMapping[projectOnChain.projectState]}
      <FundingInfo
        projectOnChain={projectOnChain}
        ...
        <div className={`tab-content ${activeTabIndex === 0 ? "active" : ""}`}>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: marked.parse(`${projectOnChain.description}`),
            }}
          ></div>
          <ul className="project-details">
            ...
            <li className="project-detail">
              ...
              >
                ${projectOnChain.currencyId}
              </span>{" "}
              <span id="funds-required">
                {projectOnChain.requiredFundsFormatted.toLocaleString()}
              </span>
            ...
            <li className="project-detail">
              <span className="detail-value" id="project-website">
                <a
                  href={projectOnChain.website}
                  target="_blank"
                  rel="noreferrer"
                >
                  {projectOnChain.website}
                </a>
              </span>
            </li>
          </ul>
