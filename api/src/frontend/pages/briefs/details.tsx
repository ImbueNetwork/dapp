...
import { type Brief, type User } from "../../models";
...
export const BriefDetails = ({ brief }: BriefProps): JSX.Element => {
  const [browsingUser, setBrowsingUser] = useState<User | null>();
  const [targetUser, setTargetUser] = useState<User | null>(null);
  const [showMessageBox, setShowMessageBox] = useState<boolean>(false);
  const isOwnerOfBrief =
    browsingUser != null && browsingUser.id == brief.user_id;
...
document.addEventListener("DOMContentLoaded", async (event) => {
  ...
  const briefId = paths.length >= 2 && parseInt(paths[paths.length - 2]);
