...
import { Brief, Currency, Project, type User } from "../../models";
...
export interface HirerDashboardProps {
  user: User;
}
      ...
      <div className="section">
        <div className="container">
          ...
          {acceptedBriefs.map((brief) => {
            const numberOfApprovedMilestones = brief.milestones.filter(
              (milestone) => milestone.is_approved
            ).length;
            const totalMilestoneCount = brief.milestones.length;
            const percent =
              (numberOfApprovedMilestones / totalMilestoneCount ?? 0) * 100;
            const timePosted = timeAgo.format(new Date(brief.created));
            return (
              <>
                <div className="brief-info">
                  <div className="brief-description">
                    <h3>{brief.headline}</h3>
                    <span>
                      {Number(brief.project.required_funds).toLocaleString()} $
                      {Currency[brief.project.currency_id]}
                    </span>
                    <span>Created {timePosted}</span>
                  </div>
                  <div className="project-milestones">
                  ...
                  <div className="brief-button">
                    <button
                      className="primary-btn in-dark w-button"
                      onClick={async () => {
                        await redirectToProjectDetails(brief.project_id);
                      }}
                    >
                      View Status
                    </button>
