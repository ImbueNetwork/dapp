...
export default class Choice extends HTMLElement {
  ...
  accountChoice(address: string | null) {
    const accountActionEntry =
      (resolve: CallableFunction) => (account: InjectedAccountWithMeta) =>
        [
          account.address,
          {
    ...
    return new Promise((resolve, _reject) => {
      ...
      if (address) {
        ...
        this.dispatchNotification("No accounts found", "", {
          dismiss: {
            label: "Okay",
          },
        });
        ...
        this.dispatchNotification(
          ...
          {
            ...Object.fromEntries(
              this.accounts.map(accountActionEntry(resolve))
            ),
            cancel: { label: "Cancel" },
          }
        );
