    ...
    this.dispatchEvent(
      new CustomEvent(config.event.notification, {
        ...
        detail: {
          ...
          actions: {
            web3: {
              handler: () => this.web3AuthWorkflow("begin", { callback }),
            },
            ...opts?.actions,
          },
          isDismissable: false,
  ...
  async web3AuthWorkflow(
    ...
    switch (event) {
      ...
      case "signed":
        {
          // post to callback
          const signature = state?.signature as SignerResult;
      ...
      case "account-chosen":
        {
          ...
          if (resp.ok) {
            ...
            if (signature) {
              return this.web3AuthWorkflow("signed", {
                ...state,
                signature,
                user,
              });
            ...
            new CustomEvent(config.event.accountChoice, {
              ...
              detail: {
                callback: (account?: InjectedAccountWithMeta) => {
                  if (account) {
                    this.web3AuthWorkflow("account-chosen", {
                      ...state,
                      account,
                    });
                  }
