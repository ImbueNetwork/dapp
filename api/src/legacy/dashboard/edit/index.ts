...
export default class Edit extends HTMLElement {
  ...
  constructor() {
    ...
    this.$submitProposal = this[CONTENT].getElementById(
      "submit-proposal"
    ) as HTMLInputElement;
    this.$categorySelect = this[CONTENT].getElementById(
      "category-select"
    ) as HTMLSelectElement;
    this.$web3AccountSelect = this[CONTENT].getElementById(
      "web3-account-select"
    ) as HTMLSelectElement;
    this.$addMilestone = this[CONTENT].getElementById(
      "add-milestone"
    ) as HTMLInputElement;
    this.$grantSubmissionForm = this[CONTENT].getElementById(
      "grant-submission-form"
    ) as HTMLFormElement;
    this.$milestoneItemTemplate = this[CONTENT].getElementById(
      "milestone-item-template"
    ) as HTMLTemplateElement;
    this.$milestones = this[CONTENT].getElementById(
      "milestones"
    ) as HTMLOListElement;
    this.$web3Address = this[CONTENT].getElementById(
      "web3-address"
    ) as HTMLInputElement;
    this.$currencySelect = this[CONTENT].getElementById(
      "currency-select"
    ) as HTMLSelectElement;
  }
  ...
  async init(request: ImbueRequest) {
    ...
    this.$web3AccountSelect.addEventListener("closed", (e) => {
      if (this.$web3AccountSelect.value === "other") this.$web3Address.focus();
    });
    ...
    const currencies = Object.keys(this.currency).filter(
      (v) => !isNaN(Number(v))
    );
  ...
  async setupProject(projectId: string) {
    ...
    try {
      draft = await fetch(`${config.apiBase}/projects/${projectId}`, {
      ...
      throw new Error(`Server error fetching project with id ${projectId}`, {
  ...
  bind() {
    ...
    this.$addMilestone.addEventListener("click", (_) => {
  ...
  proposalFromForm(formData: FormData): DraftProposal {
    return {
      ...
      required_funds:
        parseInt(formData.get("imbu-funds-required") as string) * 1e12,
  ...
  proposalIntoForm(proposal: DraftProposal): void {
    const $inputFields: Record<string, HTMLInputElement> = this.$fields.reduce(
      (p, c: Element) => ({
        ...p,
        [c.getAttribute("name") as string]: c,
      }),
      {}
    );
    ...
    this.accounts?.then((accounts) => {
      if (
        proposal.owner &&
        accounts.some((account) => account.address === proposal.owner)
      ) {
    ...
    for (let milestone of proposal.milestones) {
      ...
      $inputs["milestone-percent-to-unlock"].value = String(
  ...
  async handleFormSubmission(): Promise<void> {
    ...
    const valid = this.$fields.every(($input) => $input.checkValidity());
  ...
  reportMilestoneImbalances(submitting: boolean = false) {
    ...
    const valid = submitting ? aggregate === 100 : aggregate <= 100;
  ...
  addMilestoneItem() {
    ...
    const frag = this.$milestoneItemTemplate.content.cloneNode(
      true
    ) as DocumentFragment;

    (
      frag.querySelector(".milestone-next-label") as HTMLElement
    ).innerText = `What is your ${ordinal} milestone?`;
  ...
  get milestones(): DraftMilestone[] {
    return Array.from(this.$milestones.querySelectorAll(".milestone")).map(
      ($li: Element) => {
        const [$name, $percentageToUnlock] = Array.from(
          $li.children
        ) as HTMLInputElement[];
        return {
  ...
  wrapAuthentication(action: CallableFunction) {
    ...
    this.dispatchEvent(
      new CustomEvent(config.event.authenticationRequired, {
        bubbles: true,
        composed: true,
        detail: {
          callback,
