...
export default class Detail extends HTMLElement {
  ...
  constructor() {
    ...
    this.$tabContentContainer = this[CONTENT].getElementById(
      "tab-content-container"
    ) as HTMLElement;

    this.$tabBar = this[CONTENT].getElementById("tab-bar") as HTMLElement;

    this.tabBar = new MDCTabBar(this.$tabBar);

    this.$projectName = this[CONTENT].getElementById(
      "project-name"
    ) as HTMLElement;

    this.$projectWebsite = this[CONTENT].getElementById(
      "project-website"
    ) as HTMLElement;

    this.$projectDescription = this[CONTENT].getElementById(
      "project-description"
    ) as HTMLElement;
    this.$projectLogo = this[CONTENT].getElementById(
      "project-logo"
    ) as HTMLImageElement;

    this.$milestones = this[CONTENT].getElementById(
      "milestones"
    ) as HTMLOListElement;

    this.$fundsRequired = this[CONTENT].getElementById(
      "funds-required"
    ) as HTMLElement;

    this.$contributionSubmissionForm = this[CONTENT].getElementById(
      "contribution-submission-form"
    ) as HTMLFormElement;

    this.$projectCurrency = this[CONTENT].getElementById(
      "project-currency"
    ) as HTMLElement;

    this.$projectDetailCurrency = this[CONTENT].getElementById(
      "project-detail-currency"
    ) as HTMLElement;

    this.$fundingRoundNotYetOpenMsg = this[CONTENT].getElementById(
      "funding-round-not-yet-open"
    ) as HTMLElement;

    this.$imbuContribution = this[CONTENT].getElementById(
      "imbu-contribution"
    ) as HTMLElement;

    this.$contribute = this[CONTENT].getElementById(
      "contribute"
    ) as HTMLButtonElement;

    this.$voteSubmissionForm = this[CONTENT].getElementById(
      "vote-submission-form"
    ) as HTMLFormElement;

    this.$voteMilestoneSelect = this[CONTENT].getElementById(
      "vote-milestone-select"
    ) as HTMLSelectElement;

    this.$vote = this[CONTENT].getElementById("vote") as HTMLButtonElement;

    this.$submitMilestoneForm = this[CONTENT].getElementById(
      "submit-milestone-form"
    ) as HTMLFormElement;

    this.$submitMilestoneSelect = this[CONTENT].getElementById(
      "submit-milestone-select"
    ) as HTMLSelectElement;

    this.$submitMilestone = this[CONTENT].getElementById(
      "submit-milestone"
    ) as HTMLButtonElement;

    this.$withdraw = this[CONTENT].getElementById(
      "withdraw"
    ) as HTMLButtonElement;
  }
    ...
    return document.createRange().createContextualFragment(`
            <mwc-list-item
             ...
             value="${milestone.milestoneKey}">
                ...
                <span class="select-source" slot="secondary">${milestone.percentageToUnlock}%</span>
            </mwc-list-item>
        `);
  }
  ...
  async handleButtonStates() {
    if (this.apiInfo) {
      const projectOnChain: any = await (
        await this.apiInfo?.imbue?.api.query.imbueProposals.projects(
      ...
      if (projectOnChain) {
        ...
        const rounds: any =
          await await this.apiInfo?.imbue?.api.query.imbueProposals.rounds.entries();
        ...
        projectOnChain.milestones.forEach((milestone: any) => {
          this.$voteMilestoneSelect.appendChild(
            this.milestoneFragment(milestone)
          );
          this.$submitMilestoneSelect.appendChild(
            this.milestoneFragment(milestone)
          );
        });
          ...
          if (
            ...
            if (
              ...
              readableRound.roundType ==
                model.RoundType[model.RoundType.ContributionRound]
            ) {
              ...
              readableRound.roundType ==
                model.RoundType[model.RoundType.VotingRound]
            ) {
        ...
        if (projectOnChain.fundingThresholdMet) {
          ...
          if (this.userIsInitiator) {
            ...
            if (projectInVotingRound) {
              ...
              this.$fundingRoundNotYetOpenMsg.innerHTML =
                "Awaiting milestone approval. Milestone voting round not complete or approved. Please <a href='https://discord.gg/jyWc6k8a'>contact the team</a> for review.";
            } else {
            ...
            this.$fundingRoundNotYetOpenMsg.hidden = false;
            this.$fundingRoundNotYetOpenMsg.innerText =
              "Awaiting milestone submission";
          }
          ...
          if (
            ...
            this.$fundingRoundNotYetOpenMsg.innerHTML =
              "Awaiting funding approval. Funding round not complete or approved. Please <a href='https://discord.gg/jyWc6k8a'>contact the team</a> for review.";
            ...
            if (projectInContributionRound) {
              ...
              this.$fundingRoundNotYetOpenMsg.innerHTML =
                "Your proposal has been created successfully. Please <a href='https://discord.gg/jyWc6k8a'>contact the team</a> for review and to open your funding round.";
            }
          } else {
            this.$fundingRoundNotYetOpenMsg.innerText =
        ...
        for (
          var i = Object.keys(projectOnChain.contributions).length - 1;
          i >= 0;
          i--
        ) {
          this.user?.web3Accounts.forEach((web3Account) => {
            if (
              web3Account.address ==
                projectOnChain.contributions[i].accountId &&
              this.project
            ) {
              const contributionValue =
                BigInt(
                  projectOnChain.contributions[i].value.replaceAll(",", "")
                ) / BigInt(1e12);
              this.$fundingRoundNotYetOpenMsg.innerText =
                this.$fundingRoundNotYetOpenMsg.innerText +
                `\n\n Your Contributions: $${
                  model.Currency[this.project.currency_id]
                } ${contributionValue}`;
            }
          });
        }
  ...
  async renderProject(draft: DraftProposal | Proposal) {
    if (!draft) {
      throw new Error("Attempt to render nonexistent draft.");
    }
    ...
    this.$projectWebsite.innerHTML = `
            <a href="${draft.website}" target="_blank">${draft.website}</a>
        `;
    ...
    this.$projectCurrency.innerText =
      "$" + model.Currency[draft.currency_id as any];
  ...
  async contribute(
    ...
    const contribution = BigInt(
      parseFloat(formData.get("imbu-contribution") as string) * 1e12
    );
    ...
    switch (event) {
      case "begin": {
        ...
        return this.contribute("extrinsic-created", {
          ...state,
          extrinsic,
        });
      }
            ...
            new CustomEvent(config.event.accountChoice, {
              ...
              detail: {
                ...
                callback: (account?: InjectedAccountWithMeta) => {
                  if (account) {
                    this.contribute("account-chosen", {
                      ...state,
                      account,
                    });
                  }
      ...
      case "account-chosen":
        {
          const extrinsic = state?.extrinsic as SubmittableExtrinsic<
            "promise",
            ISubmittableResult
          >;
          const account = state?.account as InjectedAccountWithMeta;
          const injector = await web3FromSource(account.meta.source);

          const txHash = await extrinsic
            .signAndSend(
              account.address,
                ...
                api?.query.system.events((events: any) => {
                  if (events) {
                    ...
                    events.forEach((record: any) => {
                      ...
                      const contributionSucceeded =
                        `${event.section}:${event.method}` ==
                        "imbueProposals:ContributeSucceeded";
                      const [dispatchError] = event.data as unknown as ITuple<
                        [DispatchError]
                      >;
                      ...
                      if (contributionSucceeded) {
                        ...
                        const contributionProjectId = parseInt(
                          event.data[1].toString()
                        );

                        if (
                          contributionAccountId == account.address &&
  ...
  async vote(
    ...
    const userVote =
      (formData.get("vote-select") as string).toLowerCase() == "true";
    const milestoneKey = parseInt(
      formData.get("vote-milestone-select") as string
    );
    const api = this.apiInfo?.imbue?.api;
    ...
    switch (event) {
      case "begin": {
        ...
        return this.vote("extrinsic-created", { ...state, extrinsic });
      }
      case "extrinsic-created":
        {
          this.dispatchEvent(
                  ...
                  if (events) {
                    ...
                    events.forEach((record: any) => {
                      ...
                      if (voteSucceeded) {
                        ...
                        const voterProjectId = parseInt(
                          event.data[1].toString()
                        );
                        if (
                          voterAccountId == account.address &&
                          voterProjectId == this.project?.chain_project_id &&
                          this.project &&
                          this.projectId
                        ) {
                          this.$vote.classList.remove("blob");
  ...
  async submitMilestone(
    ...
    const milestoneKey = parseInt(
      formData.get("submit-milestone-select") as string
    );
    const api = this.apiInfo?.imbue?.api;
    ...
    switch (event) {
      case "begin": {
        ...
        return this.submitMilestone("extrinsic-created", {
          ...state,
          extrinsic,
        });
      }
          ...
          this.dispatchEvent(
            new CustomEvent(config.event.accountChoice, {
              ...
              detail: {
                ...
                callback: (account?: InjectedAccountWithMeta) => {
                  if (account) {
                    this.submitMilestone("account-chosen", {
                      ...state,
                      account,
                    });
                  }
                  ...
                  if (events) {
                    ...
                    events.forEach((record: any) => {
                      ...
                      const milestoneSubmitted =
                        `${event.section}:${event.method}` ==
                        "imbueProposals:MilestoneSubmitted";
                      const [dispatchError] = event.data as unknown as ITuple<
                        [DispatchError]
                      >;
                      if (dispatchError.isModule) {
                      ...
                      if (milestoneSubmitted) {
                        const types = event.typeDef;
                        const submittedProjectId = parseInt(
                          event.data[0].toString()
                        );

                        const submittedMilestoneId = parseInt(
                          event.data[1].toString()
                        );
                        ...
                        if (
                          submittedProjectId ==
                            this.project?.chain_project_id &&
                          submittedMilestoneId == milestoneKey
                        ) {
                          this.$submitMilestone.classList.remove("blob");
                          this.$submitMilestone.disabled = false;
                          this.$submitMilestone.classList.add("finalized");
                          this.$submitMilestone.innerText =
                            "Milestone Submitted";
                        }
                      }
                    });
  ...
  async withdraw(
    ...
    switch (event) {
      case "begin": {
        ...
        return this.withdraw("extrinsic-created", {
          ...state,
          extrinsic,
        });
      }
                  ...
                  if (events) {
                    ...
                    events.forEach((record: any) => {
                      ...
                      const withdrawSuccessful =
                        `${event.section}:${event.method}` ==
                        "imbueProposals:ProjectFundsWithdrawn";
                      const [dispatchError] = event.data as unknown as ITuple<
                        [DispatchError]
                      >;
                      if (dispatchError.isModule) {
                      ...
                      if (withdrawSuccessful) {
                        ...
                        const withdrawalProjectId = parseInt(
                          event.data[1].toString()
                        );

                        if (
                          withdrawalAccountId == account.address &&
                          withdrawalProjectId == this.project?.chain_project_id
                        ) {
                          this.$withdraw.classList.remove("blob");
                          this.$withdraw.disabled = false;
