...
export default class Preview extends HTMLElement {
  ...
  constructor() {
    ...
    this.$edit = this[CONTENT].getElementById("edit") as HTMLAnchorElement;
    this.$save = this[CONTENT].getElementById("save") as HTMLButtonElement;
    this.$finalize = this[CONTENT].getElementById(
      "finalize"
    ...
    this.$currencyLabel = this[CONTENT].getElementById(
      "currency-label"
    ) as HTMLElement;
      ...
      case "begin":
        {
          ...
          return this.finalizeWorkflow("extrinsic-created", {
            ...state,
            extrinsic,
          });
        }
            ...
            new CustomEvent(config.event.accountChoice, {
              ...
              detail: {
                ...
                callback: async (account?: InjectedAccountWithMeta) => {
                  if (account) {
                    this.finalizeWorkflow("account-chosen", {
                      ...state,
                      account,
                    });
                  }
                  ...
                  if (events) {
                    ...
                    events.forEach((record: any) => {
                      ...
                      const projectCreated =
                        ...
                        "imbueProposals:ProjectCreated";
                      if (projectCreated) {
                        const types = event.typeDef;
                        const createdAccountId = event.data[0];
                        const createdProjectId = parseInt(
                          event.data[2].toString()
                        );
                        if (
                          createdAccountId == account.address &&
                          this.project
                        ) {
                          this.project.chain_project_id = createdProjectId;
                          this.updateGrantProposal(
                            this.project,
                            this.project.id
                          );
                        }
