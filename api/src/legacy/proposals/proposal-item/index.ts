...
export default class Item extends HTMLElement {
  ...
  constructor(proposal: Proposal) {
    ...
    this.$img = this[CONTENT].getElementById("img") as HTMLImageElement;
    this.$name = this[CONTENT].getElementById("name") as HTMLElement;
  ...
  get href() {
    if (
      this.proposal.chain_project_id ||
      this.proposal.chain_project_id === 0
    ) {
